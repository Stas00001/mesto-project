...
export default class Card {
  constructor(data, selector, idUser, { handleCardClick, deleteCardServer, addLikeServer, removeLikeServer }) {
    ...
    // функционал карточки
    this._addLike = addLikeServer; // функция лайка
    this._removeLike = removeLikeServer;
  ...
  generate() {
    ...
    this._checkButtonHeart(this._likes) && this._likeButton.classList.add('publications__btnlike_active')
    this._numderLikes.textContent = this._countLikes(this._likes)
  ...
  _checkButtonHeart(data) { // : проверка собственника
    likes.find(like => data.like._id === this._idUser)
  }
  ...
  indicateLike(card) {         // : отображение лайка на странице
    this._numberLikes.textContent = this._countLikes(card);
